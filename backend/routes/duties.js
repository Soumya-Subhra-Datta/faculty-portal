...
// Cancel duty (soft delete - cancels duty and related substitutions)
router.delete('/:id', async (req, res) => {
  try {
    const dutyId = req.params.id;
    const io = req.app.get('io');
    
    // Get the duty details first
    const duty = await getSingleRow(
      'SELECT d.*, f.name as faculty_name FROM duties d JOIN faculty f ON d.faculty_id = f.id WHERE d.id = ?',
      [dutyId]
    );
    ...
    // Get related substitutions before canceling
    const relatedSubstitutions = await getMultipleRows(
      `SELECT s.*, sf.name as substitute_name, t.subject, t.year, t.section
       FROM substitutions s
       JOIN faculty sf ON s.substitute_faculty_id = sf.id
       JOIN timetable t ON s.timetable_id = t.id
       WHERE s.duty_id = ? AND s.status = 'confirmed'`,
      [dutyId]
    );
    
    // Cancel the duty
    await executeQuery(
      'UPDATE duties SET status = "cancelled" WHERE id = ?',
      [dutyId]
    );
    
    // Cancel related substitutions
    for (const sub of relatedSubstitutions) {
      await executeQuery(
        'UPDATE substitutions SET status = "cancelled" WHERE id = ?',
        [sub.id]
      );
      
      // Notify substitute that substitution is cancelled
      await executeQuery(
        `INSERT INTO notifications (faculty_id, title, message, type) VALUES (?, ?, ?, ?)`,
        [
          sub.substitute_faculty_id,
          'Substitution Cancelled',
          `The substitution for ${sub.subject} (${sub.year} ${sub.section}) on ${sub.substitution_date} has been cancelled. The original faculty will take the class.`,
          'substitution'
        ]
      );
      
      // Send real-time notification to substitute
      if (io) {
        io.to(`faculty_${sub.substitute_faculty_id}`).emit('notification', {
          title: 'Substitution Cancelled',
          message: `The substitution for ${sub.subject} has been cancelled`,
          type: 'substitution'
        });
      }
    }
    
    // Notify original faculty that their duty is cancelled and they should take their class
    await executeQuery(
      `INSERT INTO notifications (faculty_id, title, message, type) VALUES (?, ?, ?, ?)`,
      [
        duty.faculty_id,
        'Duty Cancelled',
        `Your ${duty.duty_type} duty on ${duty.duty_date} has been cancelled. Please take your scheduled classes.`,
        'duty'
      ]
    );
    
    // Send real-time notification to original faculty
    if (io) {
      io.to(`faculty_${duty.faculty_id}`).emit('notification', {
        title: 'Duty Cancelled',
        message: `Your duty has been cancelled. Please take your scheduled classes.`,
        type: 'duty'
      });
    }
    ...
    // Notify admin
    if (io) {
      io.to('admin_room').emit('duty_cancelled', {
        dutyId: duty.id,
        facultyName: duty.faculty_name,
        substitutionsCancelled: relatedSubstitutions.length
      });
    }
